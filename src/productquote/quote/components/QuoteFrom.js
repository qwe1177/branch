...
class QuoteFrom extends React.Component {
	...
	this.rowSelection = {
		...
		componentDidMount() {
			...
			var params = {pageNo:''};
			axios.get(connect_url + '/v1/quotation/queryQuotationUploader.do', {params: params}).then((res)=> {
		...
		quotationuodown=(e)=>{
			...
			window.location.href = connect_url + '/v1/quotation/exportQuotationData.do?token=' + token+'&quotationId='+e;
		}
		//刷新列表信息
		subsellist =() =>{
			var {parlist, pagination} =this.state;
				...
				setTimeout(() => {
					axios.get(connect_url + '/v1/quotation/viewQuotations.do', {params: parlist}).then((res)=> {
		...
		ModalhandleOk=()=>{
			...
			axios.get(connect_url + '/v1/quotation/delQuotationById.do?', {params: params}).then((res)=> {
				let code=res.data.code;
				if(code ==1)
				{
		...
		quoteSelect=(e)=>{
			...
			this.props.form.validateFields((err, values) => {
				...
				if (!err) {
					...
					axios.get(connect_url + '/v1/quotation/viewQuotations.do', {params: params}).then((res)=> {
						console.log(res);
						var data=res.data.data;
						//console.log(data);
						if(res.data.code==1){
							...
							<FormItem >
								{getFieldDecorator('type', { initialValue: '企业名称' })(
									<Select style={{ width: '100%' }}  >
										...
										<Option value="报价单编号">报价单编号</Option>
