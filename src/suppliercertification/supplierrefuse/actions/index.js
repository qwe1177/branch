...
export const queryTableData = (data) => async (dispatch, getState) => {
    try {
        ...
        if (queryform.refuseDate && queryform.refuseDate.length > 0) {
            queryform.approvalStartTime = queryform.refuseDate[0].format("YYYY-MM-DD");
            queryform.approvalEndTime = queryform.refuseDate[1].format("YYYY-MM-DD");
        }
        ...
        var params = { ...queryform, ...paramPagination, state: '3', token, moduleId }; //查询条件和分页条件传入
        debugger;
