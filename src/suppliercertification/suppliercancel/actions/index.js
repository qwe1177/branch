...
export const queryTableData = (data) => async (dispatch, getState) => {
    try {
        ...
        if (queryform.cancelDate && queryform.cancelDate.length > 0) {
            queryform.approvalStartTime = queryform.cancelDate[0].format("YYYY-MM-DD");
            queryform.approvalEndTime = queryform.cancelDate[1].format("YYYY-MM-DD");
        }
        ...
        var params = { ...queryform, ...paramPagination, state: '4', token, moduleId }; //查询条件和分页条件传入
        debugger;
