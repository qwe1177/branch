...
class DetailsFrom2 extends React.Component {
    state = {
    ...
    render() {
      return (
         ...
         footer={() =><div>
         <Button className="editable-add-btn" type="primary" onClick={this.handleAdd}>删除</Button>
         <Button className="editable-add-btn"  onClick={this.handleAdd}>修改</Button>
         <Button className="editable-add-btn"  onClick={this.handleAdd}>增加</Button>
         <Button className="editable-add-btn"  onClick={this.handleAdd}>批量上传</Button>
         <span className="editable-add-btn"><a>下载Excel模板</a> （必须按模板格式并且文件大小不得超过8M）</span>
         </div>}
         />
      </div>
			)
