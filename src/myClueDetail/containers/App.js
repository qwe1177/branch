...
import { fetchMainData,fetchMainData2,doChangeChargeMen,toOpenUpdateXSJB,toCloseUpdateXSJB,QXPK,QRTJ,QXADDMY,QRADDMY,PKreset,QRADDMYHANDLE} from '../../components/common/supplierdetails/redux';
import {doFormAdd} from '../../components/publicFollowUp/redux'
import {doQueryFollow} from '../../components/business/followupshower/redux'

@connect(
	state => ({ supplierDetailMain: state.supplierDetailMain ,EditModal: state.EditModal,followupShower: state.followupShower,editXSJBVisible:state.editXSJBVisible}),
	dispatch => bindActionCreators({ fetchMainData,fetchMainData2,doChangeChargeMen,doFormAdd,doQueryFollow ,toOpenUpdateXSJB,toCloseUpdateXSJB,QXPK,QRTJ,QXADDMY,QRADDMY,PKreset,QRADDMYHANDLE}, dispatch)
)
...
class App extends Component {
	...
	render() {
		...
		return (
			<div>
				<h3 className="page-title">我的线索详情</h3>
				<div className={isExpandClassName} >
						...
						<div className="top-wrap">
							<div className="base-wrap">
								...
								<div className={btnClassName}>
									<div className='btns'>
										...
										{ismy?<Button type="primary" className='normal'  onClick={(key,value,type) => this.addShowModal(this.supplierId,companyName,1)}>新增跟进线索</Button>:''}
										{ismy?ispkval:''}
										{ismy?<Button type="primary" className='normal' onClick={this.handleOpenChoose}>分配负责人</Button>:''}
										{theHighSeas?<Button type="primary" className='normal' onClick={this.ADDMYmodal}>加入我的线索</Button>:''}
										{isunderling?<Button type="primary" className='normal' onClick={this.openMergedWidget}>并入线索</Button>:''}
										{ismy?<Button type="primary" className='normal' onClick={this.handleFetchToHighSea}>移入公海</Button>:''}
										{ismy?<Button type="primary" className='normal' onClick={this.tourl('/productpricing/upload/')} >上传产品报价</Button>:''}

										{/* <Button type="primary" className='normal'>供应商评分</Button>
										<Button type="primary" className='normal'>供应商统计</Button> */}
										<Modal title='提示' visible={this.props.supplierDetailMain.PKVisible} onCancel={this.qxpkhandle} onOk={this.qrtjhandle} >
											<p>{this.props.supplierDetailMain.PKcontent}</p>
										</Modal >
										<Modal title='提示' visible={this.props.supplierDetailMain.addmyVisible} onCancel={this.qxADDMYhandle} onOk={this.qrADDMYhandle} >
							...
							<div className="chart-wrap">
                                <div className="xsjb">
                                    ...
                                    <p className="p2" ><span className="s1">{this.props.supplierDetailMain.data.clueLevel?this.props.supplierDetailMain.data.clueLevel:'暂无'}</span><span className="s2">{icon}</span></p>
                                    <Modal title='修改线索级别' visible={this.props.supplierDetailMain.editXSJBVisible} onCancel={this.XSJBhandleCancel} footer={null} >
                                        <WrappedEffectFrom />
                                    </Modal >
                                </div>
                            </div>
					...
					<div className="right-wrap">
						<div className="link-wrap">
							<Row>
                                <Col span={12} style={{paddingRight:'5px'}}><Button type="primary" className='normal' onClick={this.tourl('/newClue/',0)}>新建线索</Button></Col>
                                <Col span={12} style={{paddingLeft:'5px'}}><Button type="primary" className='normal' onClick={this.tourl('/editClue/')}>编辑线索</Button></Col>
</Row>
						</div>
						<Spin spinning={this.props.supplierDetailMain.isfetching}>
							<CompanyShower  />
